...
import 'package:foodybite_app/screens/recent_request.dart';
...
class _RequestState extends State<Request> {
  ...
  String longitude;
  request() async {
    String complaints = RequestController.text;
    SharedPreferences preferences = await SharedPreferences.getInstance();
    latitude = (preferences.getDouble("latitude")??0.0).toString();
    longitude = (preferences.getDouble("longitude")??0.0).toString();
    var url = "http://192.168.43.187:8000/user_request";
    ...
    var data = {
      ...
      "content": RequestController.text,
    ...
    print(data);
    var res = await http.post(url, body: data,headers: data);
    ...
    if (res.statusCode == 200) {
      Navigator.pushReplacement(
          context, MaterialPageRoute(builder: (context) => Recent_request()));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended( backgroundColor: Colors.blue[300],
        onPressed:(){
          Navigator.pushNamed(context, 'Recent_request');
        },
        label: Text('Recent Request'),
